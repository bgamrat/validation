...
use App\Entity\ContactAnnotation;

class ContactAnnotationType extends AbstractType
{
    ...
    {
        $metadata = $container
                ->getMetadataFor( 'App\Entity\ContactAnnotation' );
    ...
    public function configureOptions( OptionsResolver $resolver )
    {
        $resolver->setDefaults( array(
            'data_class' => ContactAnnotation::class
        ) );
    }
    ...
    function getHtmlPatternAndMessage( String $field, Bool $data = false )
    {
        ...
        foreach( $constraints as $c )
        {
            if( property_exists( $c, 'htmlPattern' ) )
            {
